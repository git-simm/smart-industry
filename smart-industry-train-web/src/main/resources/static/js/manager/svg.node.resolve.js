...
// ----------------------------------------------------------------------------------------------------------
    ...
    function hiddenText(map){
        var keys = ['CREC','C_PI_R','MDSK'];
        var textList = map.selectAll("text");
        if (!textList) return;
        textList.forEach(function(textContainer){
            try{
                var children = textContainer.node.children;
                var text = null;
